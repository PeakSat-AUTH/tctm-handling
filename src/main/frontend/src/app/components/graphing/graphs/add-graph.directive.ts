...
import { callMatchingMap, calls, createdRealTimeGraphsInstanceAndQualifiedNames } from 'src/app/Interfaces/GraphData';
...
constructor(private viewContainerRef: ViewContainerRef,
  ...
  private instanceHandler: InstanceHandlerService)
  {
    ...
    this.sharedService.getGraphClickEvent().subscribe((qualifiedName) => this.deleteGraph(qualifiedName));
...
makeGraph(qualifiedName: string): void{
  ...
  createdRealTimeGraphsInstanceAndQualifiedNames.push(this.yamcsInstanceName + qualifiedName);
}
...
deleteGraph(qualifiedName: string): void{
  let index = createdRealTimeGraphsInstanceAndQualifiedNames.findIndex((specificQualifiedName: string) => specificQualifiedName == qualifiedName);
